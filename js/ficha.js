...
function inserirLeituraLunarNoBloco(bloco, leituraLunar) {
  ...
  return bloco
    .split("\n")
    .map(linha => {
      if (!/leitura\s+lunar\s*:/i.test(linha.normalize("NFD").replace(/[\u0300-\u036f]/g, ""))) {
        return linha;
      }

      if (linha.includes("{{leituraLunar}}")) {
        return linha.replaceAll("{{leituraLunar}}", valor);
      }

      const depoisDosDoisPontos = linha.split(":").slice(1).join(":").trim();

      if (depoisDosDoisPontos) {
        return linha;
      }
    ...
    <section class="card">
      <div class="card-header">
        <div>
          ...
          <p>A ficha abaixo já acumula os emojis da semana, feedbacks, extras e Leitura Lunar semanal.</p>
        </div>
